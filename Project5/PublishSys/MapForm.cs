    ...
    { 
        ...
        private void Icon_DoubleClick(object sender, EventArgs e, string iconguid)
        {
            ...
            if (tmp.Parent == this.flowLayoutPanel1)
            {
                ...
                string sql = "update ICONDUIYING_H0001Z000E00 set ISDELETE = 1, S_UDTIME = '"
                    ...
                    + iconguid + "' and LEVELGUID = '" + levelguid + "' and UNITID = '" + unitid + "'";
            ...
            else
            {
                ...
                string sql = "select PGUID from ICONDUIYING_H0001Z000E00 where ICONGUID = '" + iconguid
                    + "' and LEVELGUID = '" + levelguid + "' and UNITID = '" + unitid + "'";
        ...
        private void 全选ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            foreach (ucPictureBox item in flp.Controls)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    sql = "update ICONDUIYING_H0001Z000E00 set ISDELETE = 0, S_UDTIME = '"
                        + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where ICONGUID = '"
                        + item.IconPguid + "' and LEVELGUID = '" + levelguid + "' and UNITID = '" + unitid + "'";
                    ahp6.ExecuteSql(sql, null);
                }
                else
                {
        ...
        private void checkedListBox1_Leave(object sender, EventArgs e)
        {
            ...
            if (pNode == null)
                return;
            string levelguid = pNode.Tag.ToString();
            string sql = "select MAPLEVEL from MAPDUIYING_H0001Z000E00 where ISDELETE = 0 and LEVELGUID = '"
            ...
            if (dt.Rows.Count > 0)
            {
                sql = "update MAPDUIYING_H0001Z000E00 set S_UDTIME = '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")
                    + "', MAPLEVEL = '" + maplevel + "' where ISDELETE = 0 and LEVELGUID = '" + levelguid
                    + "' and UNITEID = '" + unitid + "'";
                ahp6.ExecuteSql(sql, null);
            }
            else
