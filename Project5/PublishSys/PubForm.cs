    ...
    {        
        ...
        private void PubForm_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; ++i)
            {
                ...
                dgvr.Cells[3].Value = dt.Rows[i]["UNITNAME"].ToString() + "区域经济大数据平台";
        ...
        {
            Process p = Process.Start(WorkPath + "Publish\\DataUp.exe", "-1 1 2");
            p.WaitForExit();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("即将发布《" + pNode.Text + "区域经济大数据平台" + textBox1.Text + "》", "提示", MessageBoxButtons.OKCancel) != DialogResult.OK)
                return;
            ...
            string unitcounty = pNode.Tag.ToString();
            ...
            if (dt.Rows.Count > 0)
            {
                while (dt.Rows[0]["ULEVEL"].ToString() != "县")
                {
                    unitcounty = dt.Rows[0]["UPPGUID"].ToString();
                    sql = "select UPPGUID, ULEVEL from RG_单位注册 where ISDELETE = 0 and PGUID = '" + unitcounty + "'";
            ...
            ahp.CloseConn();
            inip.WriteString("Public", "UnitCounty", unitcounty);
            ...
            ahp = new AccessHelper(WorkPath + "data\\PublishData.mdb");
            sql = "insert into ENVIR_PUBLISH_H0001Z000E00 (PGUID, S_UDTIME, UNITID, UNITNAME, VERSION, SYSTEMNAME) values ('" +
                        pguid + "', '" + Now_Time + "', '" +
                        pNode.Tag.ToString() + "', '" + pNode.Text + "', '" + textBox1.Text + "', '" + pNode.Text + "区域经济大数据平台')";
            ahp.ExecuteSql(sql, null);
            ahp.CloseConn();
            ...
            dgvr.Cells[3].Value = pNode.Text + "区域经济大数据平台";
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            for (int i = dataGridView1.SelectedRows.Count - 1; i >= 0; --i)
            {
                ...
                sql = "update ENVIR_PUBLISH_H0001Z000E00 set ISDELETE = 1, S_UDTIME = '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where ISDELETE = 0 and PGUID = '" + pguid + "'";
                ahp.ExecuteSql(sql);
