      ...
      int ACol, int ARow, AnsiString &Value, bool &Valid)
{
    ...
    if (ARow == AdvStringGrid1 -> RowCount - 1) {
        if (Value == "")
            return;
        String sql = "insert into Nature (属性, UPGUID) values('"+ Value + "', '" + node[Now_Node].Data.PGUID + "')";
        ...
        if (Value == "") {
            String sql = "delete * from Nature where 属性 = '" + Now_Nature + "'";
            DMod -> ExecSql(sql, tempQuery);
            ...
            String sql = "update Nature set 属性 = '" + Value + "' where 属性 = '" + Now_Nature + "'";
            DMod -> ExecSql(sql, tempQuery);
        }
    }
    //ShowMessage(Value);
      ...
      int ARow, int ACol, bool &CanEdit)
{
    ...
    if (ACol == 1)
        CanEdit = false;
    Now_Nature = AdvStringGrid1->Cells[ACol][ARow];
}
//---------------------------------------------------------------------------
...
void TForm1::FindFather(TTreeNode *tnode, int &level) {
    ...
    TADOQuery *tempQuery = new TADOQuery(NULL);
    tempQuery -> Connection = DMod -> ADOConnection3;
    String sql = "select * from Nature where UPGUID = '" + pguid + "'";
...
void __fastcall TForm1::TreeViewChange(TObject *Sender, TTreeNode *Node)
{
    ...
    */
    FindFather(Node, 1);
    //AdvStringGrid1 -> Cells[0][AdvStringGrid1 -> RowCount - 1] = AdvStringGrid1 -> RowCount - 1;
