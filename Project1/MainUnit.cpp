        ...
        TMouseButton Button, TShiftState Shift, int X, int Y)
{
    //
    int row = 0, col = 0;
    int Width = AdvStringGrid2->ColWidths[0] + AdvStringGrid2->ColWidths[1] + AdvStringGrid2->ColWidths[2];
    int Height = 0;
    for (int i = 0; i < AdvStringGrid2->RowCount; ++ i)
        Height += AdvStringGrid2->RowHeights[i];
    if (Width >= X && Height >= Y)
...
void __fastcall TForm1::AdvStringGrid2EditCellDone(TObject *Sender,
      int ACol, int ARow)
{
    int A1Row = AdvStringGrid1->Row;
    if (AdvStringGrid2->Cells[1][ARow] == "")
        return;
    TADOQuery *tempQuery = new TADOQuery(NULL);
    tempQuery -> Connection = DMod -> ADOConnection3;
    String sql = "select PGUID from Nature where UPGUID = '" + node[Now_Node].Data.PGUID + "' and 属性 = '" + AdvStringGrid1->Cells[2][A1Row] + "'";
    DMod -> OpenSql(sql, tempQuery);
    if (tempQuery -> Eof) {
        ShowMessage("请先定义属性");
        AdvStringGrid2->Cells[1][ARow] = "";
        return;
    }
    if (ACol == 1)
        AdvStringGrid2->Cells[0][ARow] = ARow;
    AdvStringGrid2->AddRow();
    Form2->Edit->Text = AdvStringGrid2->Cells[1][ARow];
    Form2->PGUID = tempQuery -> FieldByName("PGUID") -> AsString;
    Form2->csDataTypeDef_ocx1->InitShow(WideString(Form2->Edit->Text),WideString(""),true) ;
    Form2->ShowModal();
    delete tempQuery;
    /*
    AdoQ = new TADOQuery(NULL);
